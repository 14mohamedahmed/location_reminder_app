...
fun sendNotification(context: Context, reminderDataItem: ReminderDataItem) {
    ...
    val stackBuilder =
        TaskStackBuilder.create(context).addParentStack(ReminderDescriptionActivity::class.java)
    ...
    val notificationPendingIntent = stackBuilder.getPendingIntent(getUniqueId(), flag)

//    build the notification object with the data to be shown
    val notification = NotificationCompat.Builder(context, NOTIFICATION_CHANNEL_ID)
        .setSmallIcon(R.mipmap.ic_launcher).setContentTitle(reminderDataItem.title)
